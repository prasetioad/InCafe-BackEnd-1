...
exports.input = async (req, res) => {
  if (req.body.length > 0) {
    ...
    for (let i in req.body) {
      data.push({ ...req.body[i], userId: decode.userId });
    }
    OrderedProduct.bulkCreate(data).then((result) => {
      formatResult(res, 201, true, "Success Make Order", result);
    });
  } else {
    if (req.body.transactionId) {
      await createOrder(req, res).then((result) => {
        formatResult(res, 201, true, "Success Make Order", result);
      });
    }
  }
